...
const SIGNIN_URL = 'http://localhost:3080/auth/signin';
...
export default function GuildsList() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch('http://localhost:3080/dashboard/@me/guilds', {
          credentials: 'include',
        });
  ...
  async function handleRefresh() {
    ...
    try {
      const res = await fetch(
        'http://localhost:3080/dashboard/@me/guilds?skipcache=true',
        { credentials: 'include' }
      );
